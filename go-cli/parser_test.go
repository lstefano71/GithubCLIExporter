...
func TestParseSession_FullSession(t *testing.T) {
	...
	if ps.Workspace.CreatedAt == nil { t.Error("Workspace.CreatedAt is nil") }
	if ps.Workspace.UpdatedAt == nil { t.Error("Workspace.UpdatedAt is nil") }
	if len(ps.Events) == 0 { t.Fatal("Events slice is empty") }
	if len(ps.Turns) == 0 { t.Fatal("Turns slice is empty") }
	hasUser, hasAssistant := false, false
	for _, turn := range ps.Turns {
		if turn.Role == "user" { hasUser = true }
		if turn.Role == "assistant" { hasAssistant = true }
	}
	if !hasUser { t.Error("no user turn found") }
	if !hasAssistant { t.Error("no assistant turn found") }
	if len(ps.Todos) == 0 { t.Fatal("Todos is empty") }
	foundDone := false
	for _, td := range ps.Todos {
		if td.ID == "t1" && td.Status == "done" { foundDone = true }
	}
	if !foundDone { t.Error("expected todo t1 with status done") }
	if len(ps.TodoDeps) == 0 { t.Fatal("TodoDeps is empty") }
	if ps.Plan == "" { t.Error("Plan is empty") }
	if len(ps.Checkpoints) == 0 { t.Fatal("Checkpoints is empty") }
	...
	if ps.ShutdownStats == nil { t.Error("ShutdownStats is nil") }
	if len(ps.Errors) == 0 { t.Error("Errors slice is empty, expected session.error events") }
...
func TestParseSession_RealSession01(t *testing.T) {
	...
	if err != nil { t.Fatalf("ParseSession returned error: %v", err) }
	if ps.Workspace.ID == "" { t.Error("Workspace.ID is empty") }
	if len(ps.Events) == 0 { t.Error("Events is empty") }
	if len(ps.Turns) == 0 { t.Error("Turns is empty") }
	if len(ps.Checkpoints) == 0 { t.Error("Checkpoints is empty") }
}
...
func TestParseSession_EmptySession(t *testing.T) {
	...
	if len(ps.Events) != 0 { t.Errorf("Events len = %d, want 0", len(ps.Events)) }
	if len(ps.Turns) != 0 { t.Errorf("Turns len = %d, want 0", len(ps.Turns)) }
	if len(ps.Todos) != 0 { t.Errorf("Todos len = %d, want 0", len(ps.Todos)) }
}

func TestParseSession_NonexistentDir(t *testing.T) {
	_, err := ParseSession("testdata/does-not-exist")
	if err == nil { t.Fatal("expected error for nonexistent directory, got nil") }
}
...
func TestParseWorkspace(t *testing.T) {
	...
	t.Run("missing file", func(t *testing.T) {
		...
		if ws.ID != "" { t.Errorf("expected empty WorkspaceMetadata, got ID=%q", ws.ID) }
	})
	t.Run("malformed YAML", func(t *testing.T) {
		...
		if ws.ID != "" { t.Errorf("expected empty WorkspaceMetadata for malformed YAML, got ID=%q", ws.ID) }
	})
}
...
func TestParseEvents(t *testing.T) {
	t.Run("full-session events", func(t *testing.T) {
		...
		if len(events) == 0 { t.Fatal("expected non-empty events") }
		typeSet := map[string]bool{}
		for _, e := range events { typeSet[e.Type] = true }
		for _, want := range []string{
			...
			if !typeSet[want] { t.Errorf("event type %q not found", want) }
		}
	})
	t.Run("empty file", func(t *testing.T) {
		events := parseEvents("testdata/empty-session/events.jsonl")
		if len(events) != 0 { t.Errorf("expected 0 events, got %d", len(events)) }
	})
	t.Run("blank lines interspersed", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "events.jsonl")
		jsonl := "\n" +
			`{"type":"user.message","id":"1","timestamp":"2026-01-15T10:00:00Z","data":{"content":"hi"}}` + "\n" +
			"\n" +
			`{"type":"assistant.message","id":"2","timestamp":"2026-01-15T10:01:00Z","data":{"content":"hello"}}` + "\n" +
			"\n"
		if err := os.WriteFile(path, []byte(jsonl), 0644); err != nil { t.Fatal(err) }
	...
	t.Run("missing file", func(t *testing.T) {
		...
		if events != nil { t.Errorf("expected nil for missing file, got %d events", len(events)) }
	})
}
